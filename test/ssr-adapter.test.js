  ...
  const adapter = createSsrAdapter({
    resolveBootstrap() {
      return [
        ...
        'await registerHydrationModules([() => import("/_nexel/static/app/components/feature-card.mjs")]);',
        "await hydratePage();",
      ].join("\n");
    },
  });
...
test("SSR adapter respects explicit framework bootstrap options over generated bootstrap", async () => {
  const adapter = createSsrAdapter({
    ...
    resolveBootstrap() {
      return 'await registerHydrationModules([() => import("/_nexel/static/app/components/feature-card.mjs")]);';
    },
  });
